...
const HEADER =
  'Course Name,Breakdown Name,Breakdown Weight (%),Drop Lowest,Downweight Count,Downweight %,Full Credit Grade (%),Sub-breakdown Name,Marks Achieved,Full Marks';
...
describe('parseCSV', () => {
  ...
  it('maps columns by header name when they are reordered', () => {
    const reordered = [
      'Full Credit Grade (%),Full Marks,Marks Achieved,Sub-breakdown Name,Course Name,Breakdown Name,Breakdown Weight (%),Drop Lowest,Downweight Count,Downweight %',
      '60,20,18,A1,CPSC 121,Assignments,30,,,',
    ].join('\n');

    const [course] = parseCSV(reordered);
    expect(course.name).toBe('CPSC 121');
    expect(course.breakdowns[0]).toMatchObject({
      name: 'Assignments',
      weight: 30,
      fullCreditGrade: 60,
    });
  ...
  describe('full credit grade column', () => {
    it('reads the threshold', () => {
      const [course] = parseCSV(csv('C,Assignments,30,,,,60,A1,59,100'));
      expect(course.breakdowns[0].fullCreditGrade).toBe(60);
    });

    it('reads a threshold of 0, which is meaningful', () => {
      const [course] = parseCSV(csv('C,Assignments,30,,,,0,A1,59,100'));
      expect(course.breakdowns[0].fullCreditGrade).toBe(0);
    });

    it('leaves a blank cell unset', () => {
      const [course] = parseCSV(csv('C,Assignments,30,,,,,A1,59,100'));
      expect(course.breakdowns[0].fullCreditGrade).toBeNull();
    });

    it('combines with a marks policy', () => {
      const [course] = parseCSV(csv('C,Assignments,30,2,,,80,A1,59,100'));
      expect(course.breakdowns[0]).toMatchObject({ dropLowestCount: 2, fullCreditGrade: 80 });
    });
  });

  describe('files exported before the full credit column existed', () => {
    /** The nine-column header shipped by the immediately previous version. */
    ...
    it('does not mistake the sub-breakdown name for a threshold', () => {
      // The column is absent, so the positional fallback lands on a text column.
      // Without a NaN guard this stored NaN and poisoned every later calculation.
      const [course] = parseCSV(
        [PREVIOUS_HEADER, 'CPSC 121,Assignments,100,,,,A1,18,20'].join('\n')
      );
      const breakdown = course.breakdowns[0];

      expect(breakdown.fullCreditGrade).toBeNull();
      expect(Number.isNaN(breakdown.fullCreditGrade as number)).toBe(false);
      expect(breakdown.subBreakdowns[0]).toMatchObject({ name: 'A1', achievedMarks: 18 });
    });

    it('imports with the grade it had before', () => {
      const [course] = parseCSV(
        [PREVIOUS_HEADER, 'C,Assignments,100,,,,A1,59,100'].join('\n')
      );
